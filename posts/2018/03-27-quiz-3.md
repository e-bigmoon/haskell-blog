---
title: Haskell Quiz No.3 QuickCheck の Arbitrary
...
```sh
>>> cycle "Haskell"
"HaskellHaskellHaskel^ClInterrupted.
```

### Prelude モジュールについて

`Prelude` モジュールは良く使う基本的な関数や型の集まりです。とても良く使うので暗黙的に `import` されています。

`Prelude` で定義されている関数の `型`・`定義`・`動作` ぐらいは全部わかるようにしておきましょう。
...
ここからの `foldr`, `foldl` は全てリストのインスタンスについての話です。
...
ということで、`cycle` は `foldr` を使って定義することは不可能です。

もっと `fold` のことを知りたい人は `A tutorial on the universality and expressiveness of fold` や関数プログラミングの楽しみの3章おりがみプログラミングを読んでみましょう。
...
`(&&)` 第一引数が `False` であれば第二引数を評価することなく `False` を返します。そのため `False && _ =  False` のような定義になっています。
