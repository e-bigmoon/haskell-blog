...
- [\@nh2](https://github.com/nh2)さんのおかげで、静的にリンクされた Linux バイナリが再び利用できるようになりまりました。
...
- `download.fpcomplete.com` から Ubuntu, Debian, CentOS, Fedora, Arch パッケージリポジトリを削除する予定です。これらは既に1年以上前から非推奨となっており、新しいリリースにも対応していませんでしたが、過去のスクリプトとの互換性のために残っています。
...
- Cabal 2.4 にアップグレードしました。
  - 注意点: ファイル名のパターンマッチ処理が変わりました。Cabal spec 2.4 より前のバージョンでは `*.txt` は `foo.txt` にマッチしていましたが、`foo.2.txt` にはマッチしませんでした。
...
- `GHCJS` のサポートが `experimental (実験的)` へ降格しました。`GHCJS` が実験段階であることをユーザに知らせるための警告が表示されます。
...
- `stack.yaml` の `ghc-options` と `config.yaml` の `ghc-options` を結合するようにしました。今までは上書きされていました。
...
- `stack build` でパッケージのサブライブラリをビルドする際に executables, test, benchmarks, libraries などと同様の方法でアナウンスを行うようになりました。
...
- `stack setup` の `--upgrade-cabal` は廃止されます。この機能は GHC 8.2 以降では動作しません。また、このフラグが実装された当初の動機は Stack がカスタムセットアップのためにスナップショットの `Caba` ライブラリを利用し始めたことでほとんど無くなりました。詳細: [#4070](https://github.com/commercialhaskell/stack/issues/4070)
...
- 新たな namespaced template 機能のため、`stack templates` は利用可能な全てのテンプレートのリストをわかりやすく表示できなくなりました。代わりに、テンプレートを探す方法についての[ヘルプファイル](https://github.com/commercialhaskell/stack-templates/blob/master/STACK_HELP.md)をダウンロードし表示します。詳細: [#4039](https://github.com/commercialhaskell/stack/issues/4039)
...
- ビルドツールを `cabal-install` と同様の方法で取り扱うようになりました。特にレガシーな `build-tools` フィールドに対しては、ツールマップの中からビルドツールパッケージを探し出すのではなく、ビルドツールのハードコードされたリストを利用します。この変更により Stack の動作は `cabal-install` に近づき、いくつかのバグを回避し、最適化/遅延性の恩恵を得られる可能性があります。詳細: [#4125](https://github.com/commercialhaskell/stack/issues/4125)
...
- `stack upload` はデフォルトでパッケージに署名を行うようになりました。`--no-signature` オプションによって署名をスキップできます。詳細: [#3739](https://github.com/commercialhaskell/stack/issues/3739)
...
- テンプレートダウンロード時のネットワーク接続問題に備えて、事前にチェックを行います。もし、問題があればキャッシュされたテンプレートを利用します。詳細: [stack new requires network to work #3850](https://github.com/commercialhaskell/stack/issues/3850)
...
- Windows 10 より前の Windows では、ターミナルのデフォルト値は `--color=never` となります。エミュレーションの出力のみ ANSI カラーコードがサポートされています。
...
- `stack build` は `stack.yaml` の `extra-deps` に `base` が明示的に記載されているとき、警告を出すようになりました。
...
- `stack build` は、ビルドプランで達成不可能な `base` バージョンが要求された際に、別の GHC バージョンを試すように提案するようになりました。
...
- 新たなサブコマンド `run` が導入されました。このコマンドは `cabal run` のように、指定された executable をビルド&実行します。もし、executable が指定されない場合、プロジェクト内で最初に利用可能な executable を利用します。
...
- `stack build` で、依存関係が足りない旨を伝えるメッセージに、パッケージの最新の cabal ファイルリビジョンハッシュを含めるようにしました。詳細: [#4068](https://github.com/commercialhaskell/stack/pull/4068)
...
- `stack sdist` に新しいオプション `--tar-dir` を追加しました。このオプションにより指定したディレクトリに tarball をコピーできるようになります。
...
- `--interleaved-output` コマンドラインオプションと `build.interleaved-output` 設定値を導入しました。これにより、複数の並行ビルドの出力が `packagename> ` プレフィックス付きで同時に標準エラー出力にダンプされます。詳細: [#3225](https://github.com/commercialhaskell/stack/issues/3225)
...
- `stack new` コマンドに `username/foo` という形式で、`commercialstack` 以外のリポジトリからテンプレートをダウンロードできるようになりました。また `github:` というプレフィックスを付けることでサービスを指定することもできます。現状は `github:` の他に `gitlab:` と `bitbucket:` が利用可能です。
...
- `'allow-newer': true` をどこに追加すれば良いかわかりづらいため、ユーザ設定ファイルへのパスをエラーメッセージに含めるようにしました。詳細: [#3685](https://github.com/commercialhaskell/stack/issues/3685)
...
- `stack ghci` はビルド後ではなく、ビルド前に、どのメインターゲットをロードするか質問するようになりました。
...
- GHC 8.4 以降のバージョンを利用したとき、自動的に Haddock へ `--quickjump` オプションを渡すようになりました。
...
- パッケージをアップロードする際に `hackage-base-url` 設定オプションを利用することで、Hackage ベースURL に任意のURLを指定できるようになりました。(デフォルトは `https://hackage.haskell.org/`)
...
- 前回のリリースで `ghc-pkg` コマンドの出力がターミナルに出力されてしまう問題が再発してしまったので修正しました。
...
- `v1.7.1` で導入された再コンパイルチェックを修正し、回帰テストを追加しました。詳細: [#4001](https://github.com/commercialhaskell/stack/issues/4001)
...
- `stack ghci` は internal library を含むパッケージの処理で internal libray に対応する間違ったパッケージを探し、モジュールの読み込みに失敗していました。この問題は現在修正され、ライブラリとサブライブラリが適切に追跡されるようにコードが修正されました。詳細: [#3926](https://github.com/commercialhaskell/stack/issues/3926)
...
- internal library に依存していないパッケージで `stack build` を実行すると、 internal library がビルドされていないのにも関わらず登録しようとするため、ビルドに失敗していました。この問題は internal library を常にビルドすることで解決しました。
...
- コマンドラインの引数の順序によって動作が異なる場合がある問題を修正しました。詳細: [#3959](https://github.com/commercialhaskell/stack/issues/3959)
...
- Hackage へアップロードする際、クレデンシャル保存時のプロンプトでユーザのレスポンスを待つ前にメッセージを画面に出力するように修正しました。ghci のターゲット選択プロンプトでも同様の問題が発生していたので修正しました。
...
- `stack solver` コマンドで、PATH に `cabal` が存在しない場合、ユーザに対して `stack install cabal-install` を実行させるためのメッセージを表示するようになりました。
...
- サブライブラリを含むパッケージが executable, test, benchmark などの依存関係になっている場合でも `stack build` でビルドできるようになりました。詳細: [#3787](https://github.com/commercialhaskell/stack/issues/3787).
...
- スナップショット更新時にサブライブラリをプリコンパイルキャシュに追加するようになりました。以前まではスナップショットにサブライブラリを含むパッケージが存在した場合、スナップショットの更新を行うとビルドが壊れていました。詳細: [#4071](https://github.com/commercialhaskell/stack/issues/4071)
...
- Cabal の呼び出しに失敗した再に "While building Setup.hs" という間違ったメッセージが表示されていた問題を修正しました。詳細: [#3934](https://github.com/commercialhaskell/stack/issues/3934)
