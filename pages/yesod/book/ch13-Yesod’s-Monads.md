...
今行っていることは, `CRandT`トランスフォーマを`HandlerT`トランスフォーマの上にレイヤリングすることである. 他の方法では機能しない: Yesodそれ自身は, 最終的に`CRand`トランスフォーマをアンラップする必要があるが, そうするための知識がないのである. これは`Persistent`でとった方法と同じであることに留意しなさい: トランスフォーマは`HandlerT`の上に位置する. 
