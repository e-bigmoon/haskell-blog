...
実際にはここに書いたよりずっと多くのことが起こっています。生成されたディスパッチコードは、効率性のためビューパターン言語拡張を利用し, さらに多くの型クラスのインスタンスが生成されます。そして、その他にサブサイトなどを制御するためのコードも同様に生成されます。これらの詳細は本書全体で解説していますが、特に "リクエストを理解する" (Understanding a Request) の章が詳しいです。
