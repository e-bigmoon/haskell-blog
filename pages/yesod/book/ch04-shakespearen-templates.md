...
展開の中では関数適用、文字列、数値リテラル、修飾付きモジュールを利用することができます。また、処理をまとめるために丸括弧やドルマークを使うこともできます。展開の処理の最後に `toHtml` 関数が適用されます。これは `ToMarkup` クラスの全てのインスタンスが展開可能だということを意味します。例えば、次のコードを考えてみましょう。
